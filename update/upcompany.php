    ...
    <div class="content-right">
        <div class="content-product">
            ...
            if (isset($_GET['action']) && $_GET['action']=='add') {
                $id3 = $_GET['id2'];
                $tennxb = $_POST['tennxb'];
                $sql3 = "UPDATE nxb SET tennxb='$tennxb' WHERE manxb=$id3";
                $result3 = mysqli_query($con, $sql3);

                if ($result3) {
                    echo 'Sua nxb thanh cong';
                }else {
                    echo 'Sua nxb khong thanh cong';
                }
                mysqli_close($con);
            }
            ...
            <div class="add-book">
                ...
                $sql4 = "SELECT * FROM nxb WHERE manxb='$id'";
                $result4 = mysqli_query($con, $sql4);
                while ($row4 = mysqli_fetch_array($result4)) {

                    ?>
                    <form action="../update/upcompany.php?action=add&id2=<?=$id?>" method="POST" enctype="multipart/form-data">

                        <div class="add-detail">
                            <label for="">Tên NXB: </label>
                            <input type="text" name="tennxb" value="<?=$row4['tennxb']?>" style="margin-left: 4.3rem;">
                        </div>
                        <div class="add-detail">
                            <input type="submit" value="Sửa NXB">
                        </div>
                    </form>
                <?php }?>
                <?php } ?>
